...
describe('verify key method', () => {
  ...
  it('invalid key', () => {
    ...
    expect(verifyKey(signedRequest.body, signedRequest.signature, signedRequest.timestamp, validKeyPair.publicKey)).toBe(false);
  });

  it('invalid body', () => {
    // Sign a valid request and verify with an invalid body
    const signedRequest = signRequestWithKeyPair(pingRequestBody, validKeyPair.secretKey);
    expect(verifyKey('example invalid body', signedRequest.signature, signedRequest.timestamp, validKeyPair.publicKey)).toBe(false);
  });

  it('invalid signature', () => {
    // Sign a valid request and verify with an invalid signature
    const signedRequest = signRequestWithKeyPair(pingRequestBody, validKeyPair.secretKey);
    expect(verifyKey(signedRequest.body, 'example invalid signature', signedRequest.timestamp, validKeyPair.publicKey)).toBe(false);
  });

  it('invalid timestamp', () => {
    // Sign a valid request and verify with an invalid timestamp
    const signedRequest = signRequestWithKeyPair(pingRequestBody, validKeyPair.secretKey);
    expect(verifyKey('example invalid body', signedRequest.signature, String(Math.round(new Date().getTime() / 1000) - 10000), validKeyPair.publicKey)).toBe(false);
  });
});