...
describe('verify key method', () => {
  ...
  it('supports array buffers', () => {
    ...
    expect(
      verifyKey(bodyEncoded.buffer, signedRequest.signature, signedRequest.timestamp, validKeyPair.publicKey),
    ).toBe(true);
  });
  ...
  it('invalid body data type', () => {
    ...
    expect(verifyKey(invalidBody, signedRequest.signature, signedRequest.timestamp, validKeyPair.publicKey)).toBe(
      false,
    );
  });
});
